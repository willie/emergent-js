...
export function MainChatPanel() {
  ...
  const {
    ...
    addCharacter, removeEventsBySourceId, removeLocationCluster,
    removeConversationsBySourceId, removeKnowledgeBySourceId,
    ...
    try {
      ...
      if (simResult) {
        ...
        for (const conv of conversations ?? []) {
          addConversation({ ...conv, sourceMessageId: messageId });
        }
        if (characterUpdates) {
  ...
  const handleRegenerate = useCallback(() => {
    ...
    // Remove simulation artifacts generated by this message
    removeEventsBySourceId(lastAssistant.id);
    removeConversationsBySourceId(lastAssistant.id);
    removeKnowledgeBySourceId(lastAssistant.id);

    // Rollback the +1 tick that handleSubmit/handleContinue always applies
    advanceTime(-1);
    ...
    if (delta) {
      ...
      if (delta.movement?.previousClusterId && currentWorld) {
        ...
        // Remove orphaned location cluster created by this movement
        if (delta.movement.isNewCluster && delta.movement.newClusterName) {
          const orphanedCluster = currentWorld.locationClusters.find(
            (c) => c.canonicalName === delta.movement!.newClusterName
          );
          if (orphanedCluster) {
            removeLocationCluster(orphanedCluster.id);
          }
        }
      }
    ...
    regenerate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoading, isSimulating, regenerate]);
